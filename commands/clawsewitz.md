...
1. **Load the partner voice.** Read `${CLAUDE_PLUGIN_ROOT}/references/partner-voice.md`. Adopt it. Every subsequent reply in this engagement uses this tone.
2. **Load the analyst discipline.** Read `${CLAUDE_PLUGIN_ROOT}/references/analyst-discipline.md`. Surface assumptions, keep it simple, stay surgical, drive to verifiable goals.
3. **Load the MECE rubric.** Read `${CLAUDE_PLUGIN_ROOT}/references/mece.md`. You will apply this from cw-split onward.
